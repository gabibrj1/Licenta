...
@Component({
  selector: 'app-presidential-round2-vote',
  templateUrl: './presidential-round2-vote.component.html',
  styleUrls: ['./presidential-round2-vote.component.scss']
})
export class PresidentialRound2VoteComponent implements OnInit, OnDestroy {
  ...
  candidateForConfirmation: any = null;
  contactInfo = '';
  ...
  constructor(
    private presidentialRound2VoteService: PresidentialRound2VoteService,
    private voteMonitoringService: VoteMonitoringService,
    private localVoteService: LocalVoteService, // pentru a găsi secția de vot - reutilizare de cod
  ...
  async ngOnInit() {
    ...
    this.presidentialRound2VoteService.checkUserVoteStatus().subscribe(
      (response) => {
  ...
  // Metoda pentru afișarea avertismentului de monitorizare
  ...
  checkEligibility(): void {
    this.isLoading = true;
    this.presidentialRound2VoteService.checkEligibility().subscribe(
      (response) => {
        ...
        if (response.eligible) {
          ...
          // Încarcă candidații din turul 2 prezidențial
          this.loadPresidentialRound2Candidates();
        }
      },
      (error) => {
        this.isLoading = false;
  ...
  loadPresidentialRound2Candidates(): void {
    this.isLoading = true;
    this.presidentialRound2VoteService.getCandidates().subscribe(
      (response) => {
        this.isLoading = false;
        this.candidates = response.candidates || [];
        
        if (this.candidates.length === 0) {
          this.error = "Nu există candidați înregistrați pentru turul 2 prezidențial.";
        } else if (this.candidates.length !== 2) {
          console.warn(`Atenție: Turul 2 ar trebui să aibă exact 2 candidați, dar s-au găsit ${this.candidates.length}`);
        }
      },
      (error) => {
        this.isLoading = false;
  ...
  // Când utilizatorul aprobă monitorizarea, trecem la buletinul de vot
  ...
  selectCandidate(candidateId: number): void {
    ...
    const isToggle = this.selectedCandidate === candidateId;
  ...
  submitVote(): void {
    if (this.selectedCandidate === null) {
      this.error = 'Trebuie să selectați un candidat pentru a putea vota.';
      return;
    }

    // Găsim candidatul selectat
    const selectedCandidate = this.candidates.find(c => c.id === this.selectedCandidate);
    if (!selectedCandidate) {
      this.error = 'Eroare la identificarea candidatului selectat.';
      return;
    }

    // Setăm candidatul pentru confirmare
    this.candidateForConfirmation = selectedCandidate;
  ...
  confirmFinalVote(): void {
    ...
    console.log('Trimit datele pentru confirmare turul 2:', requestData);
    
    this.presidentialRound2VoteService.submitVote(requestData).subscribe({
      next: (response) => {
        this.confirmationInProgress = false;
        this.showConfirmDialog = false;
        
        // Afișează un mesaj de succes
        let message = 'Votul dumneavoastră pentru turul 2 a fost înregistrat cu succes!';
        if (this.sendReceiptEmail) {
          if (this.receiptMethod === 'email') {
  ...
  cancelConfirmation(): void {
    this.showConfirmDialog = false;
    this.candidateForConfirmation = null;
    this.error = '';
