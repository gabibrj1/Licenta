...
@Component({
  ...
  templateUrl: './presidential-vote.component.html',
  styleUrls: ['./presidential-vote.component.scss']
})
export class PresidentialVoteComponent implements OnInit, OnDestroy {
  ...
  async ngOnInit() {
    ...
    this.presidentialVoteService.checkUserVoteStatus().subscribe(
      (response) => {
  ...
  checkEligibility(): void {
    this.isLoading = true;
    this.presidentialVoteService.checkEligibility().subscribe(
      (response) => {
        ...
        if (response.eligible) {
          ...
          // Încarcă candidații prezidențiali
          this.loadPresidentialCandidates();
        }
      },
      (error) => {
        this.isLoading = false;
  ...
  loadPresidentialCandidates(): void {
    this.isLoading = true;
    this.presidentialVoteService.getCandidates().subscribe(
      (response) => {
        this.isLoading = false;
        this.candidates = response.candidates || [];
        
        if (this.candidates.length === 0) {
          this.error = "Nu există candidați înregistrați pentru alegerile prezidențiale.";
        }
      },
      (error) => {
        this.isLoading = false;
  ...
  confirmFinalVote(): void {
    ...
    this.presidentialVoteService.submitVote(requestData).subscribe({
      next: (response) => {
